...
namespace SigStat.Common.Transforms
{
    ...
    public class RealisticImageGenerator2 : PipelineBase, ITransformation
    {
        ...
        /// <summary> Initializes a new instance of the <see cref="RealisticImageGenerator2"/> class with specified settings. </summary>
        /// <param name="resolutionX">Output image width.</param>
        /// <param name="resolutionY">Output image height.</param>
        public RealisticImageGenerator2(int resolutionX, int resolutionY)
        {
            w = resolutionX;
            h = resolutionY;
        ...
        {
            // The generator is calibrated to use values between 0 and 1
            xs = signature.GetFeature(X);
            var minX = xs.Min();
            var maxX = xs.Max();
            var dX = maxX - minX;
            xs = xs.Select(x => (x - minX) / dX).ToList();

            ys = signature.GetFeature(Y);
            var minY = ys.Min();
            var maxY= ys.Max();
            var dY = maxY - minY;
            ys = ys.Select(y => (y - minY) / dY).ToList();
